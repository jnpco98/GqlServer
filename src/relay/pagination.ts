...
function parsePagination(connArgs: ConnectionArgs): TPaginationMeta {
  ...
  if (!first && !last) throw new InvalidPaginationArgumentError('You must provide one of first or last');
...
export function getConnectionProperties<T>(EntityType: ClassType<T>) {
